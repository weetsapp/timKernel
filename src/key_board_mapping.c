...
void setupKeys(){
    ...
    keyLb = (keyboardMemory*)malloc(4*sizeof(keyboardMemory)); // memory allocating for out keyboard Memory 


    /*
     * keyLb + (charMemSize*index) pointing towords a memory address 
     */
    int charMemSize = sizeof(char); // in my machine is 1 byte can be used to point towords next memory address
